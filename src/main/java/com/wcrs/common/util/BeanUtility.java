...
import com.wcrs.common.anotation.EntityDisplayName;
import com.wcrs.common.anotation.EnumDescription;
import com.wcrs.common.anotation.FieldDisplayName;
...
public class BeanUtility {
	...
	public BeanUtility(BaseEntity bean) {
		init(bean);
	}
	...
	public String getEntityDisplayName() {
		if (beanDispName == null) {
			EntityDisplayName ea = clazz.getAnnotation(EntityDisplayName.class);
			if (ea != null) {
	...
	public String beanToString() {
		...
		sb_tostring.append("对象:[").append(getEntityDisplayName()).append("] ");
		...
		for (String fieldname : fieldnames) {
			...
			try {
				...
				if (obj_value instanceof Date) {
					obj_value = new SimpleDateFormat("yyyy-MM-dd")
							.format((Date) obj_value);
	...
	private boolean equalsField(String fieldName, Object obj1, Object obj2) {
		try {
			...
			if (current_value == null && obj_value == null) {
				...
				if (current_value instanceof BaseEntity
						&& obj_value instanceof BaseEntity) {// 避免递归比较,内部字段如果是BaseEntity子类则只比较pk
					return ((BaseEntity) current_value).equalsPK(obj_value);
				}
				if (current_value instanceof Date && obj_value instanceof Date) { // 日期类型比较特殊处理
					DateUtils.isSameDay((Date) current_value, (Date) obj_value);
				}
